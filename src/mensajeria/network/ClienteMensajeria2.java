...
public class ClienteMensajeria2 {
    ...
    public ClienteMensajeria2(String server, int port) {
        this.server = server;
        this.port = port;
    ...
    public static void main(String args[]) throws Exception {
        ClienteMensajeria2 ec = null;
        if (args.length == 0) {
            ec = new ClienteMensajeria2();
        } else {
            String server = args[0];
            int port = Integer.parseInt(args[1]);
            ec = new ClienteMensajeria2(server, port);
        }
        ec.init();
    }
}